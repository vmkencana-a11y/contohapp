    ...
    <div class="w-full max-w-md">
        ...
        <div class="card p-8">
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Verifikasi OTP</h2>
                <p class="text-gray-600 dark:text-gray-400">Kode OTP telah dikirim ke <strong class="text-gray-900 dark:text-white">{{ $email }}</strong></p>
            </div>
            ...
            <form action="{{ route('verify-otp.submit') }}" method="POST" class="space-y-6" x-data="otpInput()" @submit="loading = true">
                @csrf
            ...
            <div class="mt-6 text-center">
                <div class="text-sm text-gray-600 dark:text-gray-400"
                     x-data="otpResendTimer('user_login_otp_countdown', 60, {{ session('success') ? 'true' : 'false' }})">
                    <p class="mb-2">Tidak menerima kode?</p>
                    <form action="{{ route('verify-otp.resend') }}" method="POST" class="inline">
                        @csrf
                        <input type="hidden" name="email" value="{{ $email }}">
                        <button type="submit"
        ...
        <p class="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
            <a href="{{ route('login') }}" class="hover:text-primary-600">&larr; Kembali ke login</a>
        </p>
    </div>
</div>
@endsection
