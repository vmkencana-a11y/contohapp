        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-700">
            ...
            <div class="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-700 p-4">
                <div class="flex items-center justify-center gap-2">
                    ...
                    <div class="flex items-center">
                        <div class="w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center text-xs font-bold">✓</div>
                        <span class="ml-2 text-xs font-medium text-gray-600 dark:text-gray-300">OTP</span>
                    </div>
                    <div class="w-8 h-px bg-indigo-600"></div>
                    <div class="flex items-center">
                        <div class="w-6 h-6 rounded-full bg-indigo-600 text-white flex items-center justify-center text-xs font-bold ring-2 ring-indigo-100 dark:ring-indigo-900">3</div>
                        <span class="ml-2 text-xs font-bold text-indigo-600 dark:text-indigo-400">2FA</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-8">
                <div class="text-center mb-6">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">2F Authenticator</h3>
                ...
                <form action="{{ route('admin.verify-2fa.submit') }}" method="POST" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
                    @csrf
                    ...
                    <button type="submit" class="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all transform active:scale-[0.98] disabled:opacity-50 disabled:cursor-wait" :disabled="loading">
                        <span x-show="!loading">Verifikasi & Masuk Dashboard</span>
                ...
                <div class="mt-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800 rounded-xl flex gap-3">
                    ...
                    <div class="text-xs text-amber-800 dark:text-amber-200">
                        <strong>Tips:</strong> Pastikan jam di perangkat Anda sinkron dengan waktu internet untuk menghindari kegagalan kode.
                    </div>
                </div>
            </div>
        </div>
