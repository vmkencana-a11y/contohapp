...
<div class="max-w-2xl mx-auto">
    <div class="flex items-center gap-4 mb-8">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Edit Blacklist</h1>
    </div>

    <div class="card p-6">
        <form action="{{ route('admin.blacklist.update', $entry) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="type" id="type" required class="input-field w-full">
                    ...
                    <option value="{{ $type->value }}" {{ old('type', $entry->type->value) === $type->value ? 'selected' : '' }}>
                        {{ $type->label() }}
                    </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <textarea name="reason" id="reason" rows="3"
                          class="input-field w-full"
                          placeholder="Alasan pemblokiran (opsional)">{{ old('reason', $entry->reason) }}</textarea>
                @error('reason')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="expires_at" id="expires_at"
                       value="{{ old('expires_at', $entry->expires_at?->format('Y-m-d\TH:i')) }}"
                       class="input-field w-full">
